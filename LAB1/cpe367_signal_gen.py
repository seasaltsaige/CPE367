...
def gen_wav(fpath_wav_out):
	...
	num_samples = 16000
	# Frequency set to 2kHz
	f = 2000
	w1 = 2 * math.pi * f / sample_rate_hz
        # Set amplitude to 75% of max 16bit signed int
	A = 127

        f2 = 500
        w2 = 2 * math.pi * f2 / sample_rate_hz
        B = 511
	# students - well done!
	###############################################################
	###############################################################
	...
	for n in range(num_samples):
		...
		yout = A * math.cos(w1 * n + (math.pi / 4)) * B * math.cos(w2 * n - (math.pi / 4))
